...
class WeatherApi {
    ...
    function setData(): void {
        ...
        $this->metar["time"] = utcTimeToLocalTime($this->metarData["time"]["dt"]);
        $this->metar["station"] = $this->metarData["station"];
        $this->metar["sanitized"] = $this->metarData["sanitized"];

        // Temperature
        if(!empty($this->metarData["temperature"]["value"])) {
            $this->metar["temperature"] = $this->metarData["temperature"]["value"] . " °" .
                $this->metarData["units"]["temperature"];
        } else {
            $this->metar["temperature"] = 'Ingen data';
        }

        // Dewpoint
        if(!empty($this->metarData["dewpoint"]["value"])) {
            $this->metar["dewpoint"] = $this->metarData["dewpoint"]["value"] . " °" .
                $this->metarData["units"]["temperature"];
        } else {
            $this->metar["dewpoint"] = 'Ingen data';
        }

        // Density altitude
        if(!empty($this->metarData["density_altitude"])) {
            $this->metar["density_altitude"] = $this->metarData["density_altitude"] . " " .
                $this->metarData["units"]["altitude"];
        } else {
            $this->metar["density_altitude"] = 'Ingen data';
        }

        // Pressure altitude
        if(!empty($this->metarData["pressure_altitude"])) {
            $this->metar["pressure_altitude"] = $this->metarData["pressure_altitude"] . " " .
                $this->metarData["units"]["altitude"];
        } else {
            $this->metar["pressure_altitude"] = 'Ingen data';
        }

        // Humidity
        if(!empty($this->metarData["relative_humidity"])) {
            $this->metar["relative_humidity"] = round($this->metarData["relative_humidity"] * 100) . "%";
        } else {
            $this->metar["relative_humidity"] = 'Ingen data';
        }
